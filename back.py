...
1. Генерация клиента на основе JSON-шаблона (`JSON/client.json`).
2. Создание перевода на основе JSON-шаблона (`JSON/transfer.json` и др.).
...
DEFAULT_APP_SECRET = (
    "1p9Oo0zBQFNn70k8zm857GlIkA6UtaDuq6t1gKL+C0nRQt8l/tO7rlGkz0FnsO8vTwB1VGw9gB+ySNy"
)
...
def wait_for_operation_accepted(
    base_url: str,
    operation_id: str,
    cfg: AuthConfig,
    ...
    raise TimeoutError(f"Operation {operation_id} was not Accepted in time")
...
def run_scenario(
    ...
    try:
        ...
        # 3. Ждём статус Accepted (GET /v2/operations/{GUID})
        op_data = wait_for_operation_accepted(base_url, operation_id, cfg)
        print(f"[T{thread_id}] Operation {operation_id} Accepted, data={op_data}")

        # 4. Подписываем операцию (если есть эндпоинт /v2/operations/{GUID}/sign)
...
def main() -> None:
    ...
    parser.add_argument(
        "--client-template",
        type=Path,
        default=Path("JSON/client.json"),
        help="Путь к JSON-шаблону клиента.",
    )
    parser.add_argument(
        "--transfer-template",
        type=Path,
        default=Path("JSON/transfer.json"),
